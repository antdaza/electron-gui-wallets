  ...
  checkVersion() {
    return new Promise(resolve => {
      if (process.platform === "win32") {
        let antdd_path = path.join(__ryo_bin, "antdd.exe");
        let antdd_version_cmd = `"${antdd_path}" --version`;
        if (!fs.existsSync(antdd_path)) {
          resolve(false);
        }
        child_process.exec(antdd_version_cmd, (error, stdout) => {
          if (error) {
            resolve(false);
          }
          resolve(stdout);
        ...
        let antdd_path = path.join(__ryo_bin, "antdd");
        let antdd_version_cmd = `"${antdd_path}" --version`;
        if (!fs.existsSync(antdd_path)) {
          resolve(false);
        }
