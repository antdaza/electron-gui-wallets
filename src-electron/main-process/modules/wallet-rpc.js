...
export class WalletRPC {
  ...
  async handle(data) {
    ...
    switch (data.method) {
      ...
      case "get_article_metadata":
        this.getArticleMetadata(params.tx_hash);
        break;

      case "verify_article_content":
        this.verifyArticleContent(params.content, params.content_hash);
        break;

      case "search_articles":
        this.searchArticles(params.query, params.limit);
        break;
      default:
    }
  }
  ...
  finalizeNewWallet(filename) {
    ...
    publishArticle(password, title, content, publisher, address, amount, payment_id = null, priority = 1) {
    crypto.pbkdf2(password, this.auth[2], 1000, 64, "sha512", (err, password_hash) => {
      if (err) {
        this.sendGateway("article_error", {
          code: -1,
      ...
      // Validate lengths
      if (title.length > 128 || content.length > 3000 || publisher.length > 64) {
        this.sendGateway("article_error", {
          code: -1,
          i18n: "notification.errors.articleTooLong"
        });
        return;
      }
      ...
      const content_hash = crypto
        ...
        .update(content)
        .digest("hex");
      const serialized_blob = `TITLE:${title};CONTENT_HASH:${content_hash};PUBLISHER:${publisher}`;
      const extra_nonce = `ARTC${serialized_blob}`;
      ...
      const transfer_params = {
        ...
        tx_extra: extra_nonce
      };

      if (payment_id) {
        transfer_params.payment_id = payment_id;
      }

      this.sendRPC("transfer", transfer_params).then(data => {
        if (data.hasOwnProperty("error")) {
          this.sendGateway("article_error", {
            code: -1,
        ...
        this.sendGateway("article_published", {
  ...
  getArticleMetadata(tx_hash) {
    this.sendRPC("get_transfer_by_txid", { txid: tx_hash }).then(data => {
      if (data.hasOwnProperty("error")) {
        this.sendGateway("article_error", {
          code: -1,
          message: data.error.message
      ...
      const tx_extra = data.result.transfer.tx_extra;
      const artcIndex = tx_extra.indexOf("ARTC");

      if (artcIndex === -1) {
        this.sendGateway("article_error", {
          code: -1,
          i18n: "notification.errors.noArticleData"
        });
        return;
      }

      const metadataStr = tx_extra.substring(artcIndex + 4);
      ...
      if (!metadata.title || !metadata.content_hash || !metadata.publisher) {
        this.sendGateway("article_error", {
          code: -1,
          i18n: "notification.errors.invalidArticleFormat"
        });
        return;
      }

      this.sendGateway("article_metadata", {
        tx_hash,
        height: data.result.transfer.height,
        timestamp: data.result.transfer.timestamp,
        ...metadata
      });
    });
  }

  verifyArticleContent(content, content_hash) {
    const calculated_hash = crypto
      .createHash("sha3-256")
      .update(content)
      .digest("hex");

    this.sendGateway("article_verification", {
      is_valid: calculated_hash === content_hash
    });
  }
  ...
  searchArticles(query, limit = 10) {
    this.sendRPC("get_transfers", {
      ...
      min_height: 0 // Get all transfers
    }).then(data => {
      if (data.hasOwnProperty("error")) {
        this.sendGateway("article_error", {
          code: -1,
      ...
      const results = [];
      const allTransfers = [...(data.result.in || []), ...(data.result.out || [])];

      for (const transfer of allTransfers) {
        if (!transfer.tx_extra) continue;

        const artcIndex = transfer.tx_extra.indexOf("ARTC");
        if (artcIndex === -1) continue;

        const metadataStr = transfer.tx_extra.substring(artcIndex + 4);
        const parts = metadataStr.split(";");
        const metadata = {};
        ...
        if (!metadata.title || !metadata.publisher) continue;

        // Simple search
        if (query) {
          const searchText = `${metadata.title} ${metadata.publisher}`.toLowerCase();
      ...
      this.sendGateway("article_search_results", {
        results
      });
    });
  }
 ...
 getRecentArticles(limit = 3) {
    return this.sendRPC("get_transfers", {
      in: true,
      out: true,
      pool: false,
      filter_by_height: true,
      min_height: Math.max(0, (this.wallet_state.height || 0) - 1000) // Last 1000 blocks
    }).then(data => {
      if (data.hasOwnProperty("error")) {
        return { result: [] };
      }
  ...
  publishArticle2(password, title, content, publisher, address, amount, payment_id = null, priority = 1) {
    crypto.pbkdf2(password, this.auth[2], 1000, 64, "sha512", (err, password_hash) => {
      if (err) {
        this.sendGateway("set_article_status", {
          publish: {
            code: -1,
            i18n: "notification.errors.internalError",
            sending: false
      ...
      this.sendRPC("transfer", transfer_params).then(data => {
        ...
        this.sendGateway("set_article_status", {
          publish: {
            code: 0,
            i18n: "notification.positive.articlePublished",
            sending: false,
            tx_hash: data.result.tx_hash,
            content_hash
          }
        });
        // Update recent articles in next heartbeat
        this.heartbeatAction();
      });
    });
  }
